...
public class SetupFirefoxDriver {
	...
	public WebDriver setupFirefoxDriver() {
		...
		// File cDriver = new
		// File(SetupChromeDriver.class.getResource(MAC_DRIVER).getFile());
		//
		// // Is it executable
		...
		// SetupChromeDriver.class.getResource(MAC_DRIVER).getFile());
		//
		// // Now checking for existence of Chrome executable.'
		// if (!new File("/Applications/Google Chrome.app/Contents/MacOS/Google
		// Chrome").exists()) {
		// throw new RuntimeException(
		// "Cannot find chromedriver file. Please download and copy to drivers
		// folder in current project");
		// }
		// } else {
		// // Make sure Chrome is installed on the default location on your
		// // machine.
		// System.out.println(SetupChromeDriver.class.getResource(WINDOWS_DRIVER).getFile());
		// System.setProperty("webdriver.chrome.driver",
		// SetupChromeDriver.class.getResource(WINDOWS_DRIVER).getFile());
		// }
